...
const typeModels = (typeParam: string) => {
  if (typeParam === 'tryout-sections' || typeParam === 'tryout-section')
    return TryoutSection
  if (typeParam === 'courses' || typeParam === 'course') return Course
  if (typeParam === 'apps' || typeParam === 'app') return App
...
export const getFilterByIdType = async (
  req: Request,
  res: Response
): Promise<void> => {
